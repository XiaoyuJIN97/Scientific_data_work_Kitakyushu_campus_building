...
# 读取Excel文件# 读取Excel文件
file_path = r"E:\Scientific_data_work_Kitakyushu_campus_building\Data Processing\Energy station\2002\Energy station_2002.xlsx"  # 请将文件路径更新为实际路径
...
# 创建包含日期和数据的DataFrame
processed_df = pd.DataFrame({'Date': np.repeat(dates, len(times)), 'Solar energy genetation': processed_data, 'Energy receiveing': processed_data2, 'Gas engine': processed_data3, 'Fuel cell': processed_data4, 'AC': processed_data5})

# 保存处理后的数据为Excel文件
processed_df.to_excel(r'E:\Scientific_data_work_Kitakyushu_campus_building\Data Processing\Energy station\2002\Energy station_time series_2002.xlsx', index=False)
