...
# 读取Excel文件# 读取Excel文件
file_path = r"E:\Scientific_data_work_Kitakyushu_campus_building\Data Processing\Energy station\2002\Buildings electricity usage\Buildings electricity usage_2002.xlsx"  # 请将文件路径更新为实际路径
...
# 创建包含日期和时间的列名
dates = pd.date_range(start="2002-01-01", periods=365)
...
# 创建包含日期和数据的DataFrame
processed_df = pd.DataFrame({'Date': np.repeat(dates, len(times)), 'Research building': processed_data, 'Experimental building': processed_data2, 'Industry and Education Building': processed_data3, 'Communication building': processed_data4, 'GYM': processed_data5, 'Media building': processed_data6})

# 保存处理后的数据为Excel文件
processed_df.to_excel(r'E:\Scientific_data_work_Kitakyushu_campus_building\Data Processing\Energy station\2002\Buildings electricity usage\Buildings electricity usage_Time series_2002.xlsx', index=False)
