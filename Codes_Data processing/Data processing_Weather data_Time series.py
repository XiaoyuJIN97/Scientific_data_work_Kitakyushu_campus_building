...
# 读取Excel文件# 读取Excel文件
file_path = r"E:\Scientific_data_work_Kitakyushu_campus_building\Data Processing\Energy station\2002\Weather data_2002.xlsx"  # 请将文件路径更新为实际路径
...
data2 = df2.iloc[0:367, 6:30]
data3 = df3.iloc[0:367, 6:30]
data4 = df4.iloc[0:367, 6:30]
...
# 创建包含日期和数据的DataFrame
processed_df = pd.DataFrame({'Date': np.repeat(dates, len(times)), 'Horizontal solar irradition': processed_data, 'Outdoor temperature': processed_data2, 'Wind speed': processed_data3, 'Wind direction': processed_data4})

# 保存处理后的数据为Excel文件
processed_df.to_excel(r'E:\Scientific_data_work_Kitakyushu_campus_building\Data Processing\Energy station\2002\Weather data_time series_2002.xlsx', index=False)
