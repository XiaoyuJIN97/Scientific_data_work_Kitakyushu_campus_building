...
# 读取Excel文件# 读取Excel文件
file_path = r"E:\Scientific_data_work_Kitakyushu_campus_building\Data Processing\AHU & N304\Wall temperature_2022.xlsx"  # 请将文件路径更新为实际路径
...
# 创建包含日期和数据的DataFrame
processed_df = pd.DataFrame({'Date': np.repeat(dates, len(times)), '2F North room temperature (℃)': processed_data, '2F North wall temperature (℃)': processed_data2, '2F South room temperature (℃)': processed_data3, '2F South wall temperature (℃)': processed_data4})

# 保存处理后的数据为Excel文件
processed_df.to_excel(r'E:\Scientific_data_work_Kitakyushu_campus_building\Data Processing\AHU & N304\Wall temperature_Time series_2022.xlsx', index=False)
