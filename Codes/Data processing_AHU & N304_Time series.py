...
# 读取Excel文件# 读取Excel文件
file_path = r"E:\Scientific_data_work_Kitakyushu_campus_building\Data Processing\AHU & N304\AHU & N304_2009.xlsx"  # 请将文件路径更新为实际路径
...
#processed_df = pd.DataFrame({'Date': np.repeat(dates, len(times)), 'Air supply temperature': processed_data2, 'Temperature below the corridor': processed_data3, 'Humidity below the corridor': processed_data4, 'Outdoor air intake temperature': processed_data5})

# 保存处理后的数据为Excel文件
processed_df.to_excel(r'E:\Scientific_data_work_Kitakyushu_campus_building\Data Processing\AHU & N304\AHU & N304_time series_2009.xlsx', index=False)
