...
# 读取Excel文件# 读取Excel文件
file_path = r"E:\Scientific_data_work_Kitakyushu_campus_building\Data Processing\Gas\Gas usage_2022.xlsx"  # 请将文件路径更新为实际路径
...
# 创建包含日期和数据的DataFrame
processed_df = pd.DataFrame({'Date': np.repeat(dates, len(times)), 'Boiler': processed_data, 'Fuel cell': processed_data2, 'Gas engine': processed_data3, 'Absorption chiller 1': processed_data4, 'Absorption chiller 2': processed_data5, 'Absorption chiller 3': processed_data6})

# 保存处理后的数据为Excel文件
processed_df.to_excel(r'E:\Scientific_data_work_Kitakyushu_campus_building\Data Processing\Gas\Gas_Time series_2022.xlsx', index=False)
